...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="{{ $data }}"> 
    ...
    <body class="font-sans antialiased bg-gray-200 dark:bg-gray-700">
        <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
            <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-white md:w-64 dark:text-gray-200 dark:bg-gray-800 shadow-2xl" x-data="{ open: false }">
                ...
                <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto shadow-2xl">
                    ...
                    <a class="block px-4 py-2 mt-2 text-sm font-semibold text-gray-900 rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline {{ request()->is('admin/celebrities') ? 'bg-gray-200 dark:bg-gray-700' : 'bg-transparent dark:bg-transparent' }}" href="{{ route('celebrities') }}">Celebrities</a>
                    ...
                    <div @click.away="open = false" class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left rounded-lg dark:focus:text-white dark:hover:text-white dark:focus:bg-gray-600 dark:hover:bg-gray-600 md:block hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline {{ request()->is('admin/user') || request()->is('admin/role') ? 'bg-gray-200 dark:bg-gray-700' : 'bg-transparent dark:bg-transparent' }}">
                            ...
                            <i x-show="!open" class="material-icons align-middle pb-1">&#xe313;</i>
                            <i x-show="open" class="material-icons align-middle pb-1">&#xe316;</i>
                        </button>
            ...
            <div class="w-full">
                <header class="bg-white shadow-lg dark:bg-gray-900 flex items-center">
                    ...
                    <i x-show="!logout" class="material-icons align-middle pb-1">&#xe313;</i></button>

                    <!-- Dropdown menu -->
                    <div id="dropdownInformation" class="hidden bg-white text-base z-10 list-none divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-900 dark:divide-gray-600">
                        <div class="px-4 py-3 text-gray-900 dark:text-white">
                            <span class="block text-sm">{{ Auth::user()->name }}</span>
                            <span class="block text-sm font-medium truncat">{{ Auth::user()->email }}</span>
                        </div>
                        <div class="py-1">
